...
//  Added img field for the second session and also to the routes
  def createComment(text: String, hashtag: String, username: String, postID: String, img: String) = Action { implicit request =>
    var numberOfComments = post.findAllSortedByDate.length+1
    post.createPost(post(numberOfComments, username, DateTime.now, text, ListBuffer(), 0, 0, hashtag, 1, img))
...
//  def FollowHashtags(hashtag: String, Hashtag: String) = Action { implicit request =>
//    //    val Username = request.session.get("username").toList(0)
//    var users = models.user.findallUsers
//
//    users.foreach { X =>
...
// Added some parts for the second session
  def Explore = Action { implicit request =>
    ...
    usernameOption.map { username =>
      ...
      // Added for second session: save the topics as well because we want to show them
//      var hashtags = ListBuffer[String]()
//      posts.foreach(x => hashtags += x.hashtag)

      val USER = users.find(_.username == username).toList(0)

//      val HASHTAG = posts.find(_.hashtag == use)
      ...
      posts.foreach { X =>
        ...
        while (i < USER.friends.length){
          ...
          // Added for second session: show posts with the topics
          posts.find(_.hashtag == USER.friends(i)).toList.foreach{ x =>
            posts2 = posts2 :+ x
          }
